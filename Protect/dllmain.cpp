...
#define MethodEncryptionDef 0
#define CRC32CheckDef 1

#define CRC32 3598641237
#define AssemblySize 147456

#define AntiDebugManagedDelay 1000000

///////////////////////////////////

#if MethodEncryptionDef == 0
const uint8_t iv[] = {0x0};
const uint8_t key[] = {0x0};
const uint8_t Code2[] = {0x0};
#endif
...
extern "C" __declspec(dllexport) bool _stdcall Protect() {//void(__stdcall *antidump)()
	...
	if (*(UINT*)(ptr - 0x78) != 0) {
		...
		*(UINT*)newarr = 0x6c64746e;
		*((UINT*)newarr + 1) = 0x6c642e6c;
		*((USHORT*)newarr + 4) = 0x006c;
		*(newarr + 10) = 0;

		for (INT i = 0; i < 11; i++)
			*(modName + i) = *(BYTE*)(newarr + i);

		VirtualProtect(funcName, 11, 0x40, &old);

		*(UINT*)newarr = 0x6f43744e;
		*((UINT*)newarr + 1) = 0x6e69746e;
		*((USHORT*)newarr + 4) = 0x6575;
		*(newarr + 10) = 0;

		for (int i = 0; i < 11; i++)
			*(funcName + i) = *(BYTE*)(newarr + i);
	}
	...
	*(UINT*)mdHdr = 0;
	...
	VirtualProtect((LPVOID)(module_ptr + MethodEncryptionRVA), sizeof(Code1), 0x40, &old);
	ReadProcessMemory(curProc, (LPVOID)(module_ptr + MethodEncryptionRVA), &Code1, sizeof(Code1), NULL);
	...
	VirtualProtect((LPVOID)(module_ptr + MethodEncryptionRVA), sizeof(AllBytes), 0x40, &old);
	memset((void*)(module_ptr + MethodEncryptionRVA), 0x0, CodeLength1 + CodeLength2); // Set Code Section to 0x0
	WriteProcessMemory(curProc, (LPVOID)(module_ptr + MethodEncryptionRVA), &AllBytes, sizeof(AllBytes), NULL);
	VirtualProtect((LPVOID)(module_ptr + MethodEncryptionRVA), sizeof(AllBytes), old, &old);
#endif
	return true;
}
...
void eraseCode() {
	...
	//memset((void*)(module_ptr + MethodEncryptionRVA), 0x0, 4096);
	//VirtualProtect((void*)module_ptr, 4096, 0x0, &old);
	VirtualProtect((void*)(module_ptr + MethodEncryptionRVA), CodeLength1 + CodeLength2, 0x40, &old);
	memset((void*)(module_ptr + MethodEncryptionRVA), 0x0, CodeLength1 + CodeLength2);
	VirtualProtect((void*)(module_ptr + MethodEncryptionRVA), CodeLength1 + CodeLength2, 0x0, &old);
}
